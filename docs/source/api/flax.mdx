...
CryptoTensors extends the Flax API with optional encryption support via the `config` parameter in `save_file` and `save` functions.
...
from cryptotensors.flax import save_file, load_file

tensors = {
    "weight": jnp.zeros((100, 100)),
    "bias": jnp.zeros((100,))
}
