...
def format_markdown_table(df):
    ...
    for idx, row in df.iterrows():
        upstream_str = (
            f"{row['upstream']:.3f}" if not pd.isna(row["upstream"]) else "N/A"
        )
        plain_str = (
            f"{row['crypto-plain']:.3f}" if not pd.isna(row["crypto-plain"]) else "N/A"
        )
        enc_str = (
            f"{row['crypto-encrypted']:.3f}"
            if not pd.isna(row["crypto-encrypted"])
            else "N/A"
        )

        diff_plain = (
            f"{row['plain_vs_upstream_pct']:+.1f}%"
            if not pd.isna(row["plain_vs_upstream_pct"])
            else "N/A"
        )
        diff_enc = (
            f"{row['encrypted_vs_upstream_pct']:+.1f}%"
            if not pd.isna(row["encrypted_vs_upstream_pct"])
            else "N/A"
        )
...
def main():
    ...
    parser.add_argument(
        "--encrypted", help="Path to encrypted CryptoTensors benchmark JSON"
    )
    parser.add_argument(
        "--output", required=True, help="Path to save merged results JSON"
    )
    parser.add_argument(
        "--markdown", required=True, help="Path to save comparison markdown"
    )
    ...
    base_data = (
        plain_data
        if plain_data
        else (upstream_data if upstream_data else encrypted_data)
    )
